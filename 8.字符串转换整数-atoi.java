...
class Solution {
    ...
    public int myAtoi(String str) {
        ...
        int flag = 1;
        /**
         * 检测地字符串是否为可转化为数字。 2. 长度为0 3. 第一个字符不0且不为符号位
         */
        if (str.length() == 0 || (!isDigit(str.charAt(0)) && str.charAt(0) != '+' && str.charAt(0) != '-')) {
            return 0;
        }
        if (str.charAt(0) == '-') {
            flag = -1;
        ...
        if (str.charAt(0) == '+' || str.charAt(0) == '-') {
            if (str.length() == 1 || !isDigit(str.charAt(1))) {
                return 0;
            }
            str = str.substring(1);
        }
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (i == str.length() - 2
                    && (sum > INT_MAX / 10 || (sum == INT_MAX / 10 && str.charAt(i + 1) > INT_MAX % 10 + '0'))) {
                if (flag == 1) {
                    return INT_MAX;
                } else {
                    return INT_MIN;
